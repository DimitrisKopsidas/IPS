...
BEGIN
	...
	FROM 
		PROMOLINES PRL 
	INNER JOIN 
		CAROUSEL C ON C.CAROUSEL = PRL.MINIGAME
	WHERE 
		PRL.PROMO = p_promo;
	END$$
DELIMITER ;
...
BEGIN
	...
	WHERE TABLE_SCHEMA = 'crystal_v1' 
    AND TABLE_NAME = 'carousel';
END$$
DELIMITER ;

DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `GetNextProductCode`()
BEGIN
	SELECT 
...
BEGIN
    SELECT AUTO_INCREMENT+1 AS NEXTID
	FROM INFORMATION_SCHEMA.TABLES
	WHERE TABLE_SCHEMA = 'crystal_v1' 
...
INSERT INTO PROMOLINES (MINIGAME,PROMO,CHANCE) VALUES (1,1,0.4),(1,2,0.3),(1,3,0.3);
INSERT INTO ISSUEDPROMO (REDEEMCODE,PROMO,DEVICE,CAROUSEL,MINIGAME,REDEEMED,ISSUEDATE) VALUES (154165,2,1,1,1,TRUE,NOW()),(781569,2,1,1,1,FALSE,NOW()),(346518,3,1,1,1,FALSE,NOW()),(347196,1,1,1,1,FALSE,NOW()),(978745,3,2,1,1,FALSE,NOW()),(154879,1,2,1,1,FALSE,NOW());
